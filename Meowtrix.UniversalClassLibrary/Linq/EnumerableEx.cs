...
namespace Meowtrix.Linq
{
    ...
    public static class EnumerableEx
    {
        ...
        /// <summary>
        /// Take the item that has min value of key in a sequence, or default value if the sequence has no item.
        /// </summary>
        /// <typeparam name="T">Type of the items in the sequence.</typeparam>
        /// <typeparam name="TResult">Type of the key.</typeparam>
        /// <param name="source">The source sequence.</param>
        /// <param name="selector">The function to select key from items.</param>
        /// <returns>The item that has min value of key. Default value of <typeparamref name="T"/> if the sequence has no item.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="source"/> is null.</exception>
        public static T TakeMinOrDefault<T, TResult>(this IEnumerable<T> source, Func<T, TResult> selector)
            where TResult : IComparable<TResult>
        {
            if (source == null) throw new ArgumentNullException(nameof(source));
            using (var e = source.GetEnumerator())
